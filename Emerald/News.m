...
//    }
    ...
    [feedLoader setShouldAppendUserInfo:YES];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(FinishAnimation) name:@"FinishAnimation" object:nil];
}
...
-(void)viewWillAppear:(BOOL)animated{
    if ([[UserManager sharedInstance] userLoggedIn]) {
        [segmentedControl setTitle:@"人気順" forSegmentAtIndex:0];
        [segmentedControl setTitle:@"参加予定" forSegmentAtIndex:1];
        [segmentedControl setTitle:@"開催日順" forSegmentAtIndex:2];
        [segmentedControl setTitle:@"お気に入り" forSegmentAtIndex:3];
    } else {
        [segmentedControl setTitle:@"人気順" forSegmentAtIndex:0];
        [segmentedControl setTitle:@"新着順" forSegmentAtIndex:1];
        [segmentedControl setTitle:@"開催日順" forSegmentAtIndex:2];
        [segmentedControl setTitle:@"お気に入り" forSegmentAtIndex:3];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (scrollView==tableV) {
        ...
        {
            if (!isHeaderHidden && [dataTable count] > 1) {
                //Animate (close)
                [UIView beginAnimations:nil context:nil];
                [UIView setAnimationDuration:0.3];
...
-(void)UserTouchedImage:(UIButton *)sender{
    NSLog(@"UserTouchedImage %@", [dataTable objectAtIndex:(sender.tag-8888)]);
...
-(void)ClassicFeedFailedToLoadInfo:(NSError *)error{
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:[Config getStringError]
                                                   message:[Config getStringErrorMessage]
                                                  delegate:nil
...
#pragma mark SegmentedControl changes
- (IBAction)segmentChanged:(id)sender {
    [self.searchBarObj resignFirstResponder];
    
    UISegmentedControl *control = (UISegmentedControl *)sender;
    NSString *currentSource = [NSString stringWithFormat:@"%@%d", self.XMLsource, MAX(control.selectedSegmentIndex, 1)];
    [feedLoader setShouldAppendUserInfo:YES];
    [feedLoader setXMLSource:currentSource];
    [self RefreshInfoTable];
}
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar {
    ...
    NSString *searchUrl = [Config getFeedSearchUrl];
    NSString *searchSource = [NSString stringWithFormat:@"%@/%@", searchUrl, searchTxt];
    [feedLoader setShouldAppendUserInfo:NO];
    [feedLoader setXMLSource:searchSource];
    [self RefreshInfoTable];
}
